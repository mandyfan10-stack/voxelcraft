...
if (typeof window !== "undefined" && window.IS_TOUCH === undefined) {
  ...
  window.MobileInput = window.MobileInput || {
    mx: 0, mz: 0, jump: 0, mine: 0, placeTick: 0, interactTick: 0,
  };
}
...
function Joystick() {
  ...
  const stateRef = React.useRef({ activeTouch: null, cx: 0, cy: 0, dx: 0, dy: 0 });
...
function ActionButton({ label, kind, color, big, hold, onTick, style }) {
  ...
  function onEnd(e) {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
      if (e.changedTouches[i].identifier === activeRef.current) { matched = true; break; }
    }
    if (!matched) return;
    e.preventDefault();
  ...
  return (
    <button
      ...
      onTouchEnd={onEnd}
      onTouchCancel={onEnd}
      onClick={(e) => { e.preventDefault(); /* tap fallback for non-touch */ }}
      style={{
        ...
        background: pressed
          ? `radial-gradient(circle at 30% 30%, ${color || "var(--olive)"} 0%, ${color || "var(--olive-deep)"} 100%)`
          : "radial-gradient(circle at 30% 30%, rgba(43,40,35,0.92) 0%, rgba(10,9,8,0.95) 100%)",
        color: "var(--bone)",
        ...
        boxShadow: pressed
          ? `inset 0 0 0 1px rgba(0,255,255,0.4), 0 0 16px rgba(0,255,255,0.4)`
          : "inset 0 1px 0 rgba(216,210,196,0.08), 0 4px 12px rgba(0,0,0,0.7)",
        pointerEvents: "auto",
        touchAction: "none",
        userSelect: "none",
        WebkitUserSelect: "none",
